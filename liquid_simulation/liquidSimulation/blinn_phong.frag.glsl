...
// vec4 GoldColor(vec3 normal, vec3 direction){
//   vec3 w = vec3(1,1,1);
//   vec3 p = vec3(1,2,20);

//   float costeta = max(dot(normal,direction),0);

//   vec4 color = vec4(1,1,1,1.0);

//   color[0] = w[0]*pow((costeta+1)/2, p[0]);
//   color[1] = w[1]*pow((costeta+1)/2, p[1]);
//   color[2] = w[2]*pow((costeta+1)/2, p[2]);
...
void main (void){
  ...
  const vec3 eyepos = vec3(20,20,20);
  ...
  // vec4 color0 = GoldColor(normal,direction0);
  ...
  // vec4 color1 = GoldColor(normal,direction1);
